...
def smart_cat(tensor1: Optional[torch.Tensor], tensor2: torch.Tensor, dim: int) -> torch.Tensor:
    if tensor1 is None:
        return tensor2
...
def reduce_masked_mean(input: torch.Tensor, mask: torch.Tensor, dim: Optional[int] = None, keepdim: bool = False) -> torch.Tensor:
    r"""Masked mean
...
def bilinear_sampler(input: torch.Tensor, coords: torch.Tensor, align_corners: bool = True, padding_mode: str = "border") -> torch.Tensor:
...
def sample_features5d(input: torch.Tensor, coords: torch.Tensor) -> torch.Tensor:
