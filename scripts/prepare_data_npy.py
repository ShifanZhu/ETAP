...
# --------- window collection + cache save helpers ---------
def collect_window(dataset, start_us: int, end_us: int, debug: bool = False):
    """Collect frames from `dataset` whose timestamps lie in [start_us, end_us]."""
    ...
    frames_collected = 0
    last_ts_s: Optional[float] = None  # avoid overlap duplicates
    ...
    if not sample_all_voxels:
        if debug:
            print("No frames collected in window.")
        return None, None
...
def save_cache_pt(cache_dir: Path, name: str, start_us: int, end_us: int,
                  margin_us: int, vox: torch.Tensor, ts_s: torch.Tensor):
    """Save to a single .pt with int64 µs timestamps."""
    cache_dir = Path(cache_dir)
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_path = cache_dir / f"{start_us}.pt"

    ts_us = (ts_s * 1e6).round().to(torch.long)
    torch.save(
        {
            "voxels": vox.cpu(),               # [T, C, H, W]
            "timestamps_us": ts_us.cpu(),      # [T], int64
            "meta": {
                "sequence": name,
                "start_us": int(start_us),
                "end_us": int(end_us),
                "margin_us": int(margin_us),
                "frames": int(vox.shape[0]),
    ...
    print(f"💾 Saved cache: {cache_path} (frames={vox.shape[0]})")
...
def save_cache_npy(cache_dir: Path, name: str, start_us: int, end_us: int,
    ...
    base = cache_dir / f"{start_us}"
    vox_path = cache_dir / f"{start_us}_vox.npy"
    ts_path  = cache_dir / f"{start_us}_ts.npy"
    meta_path = cache_dir / f"{start_us}_meta.json"

    # convert to numpy (no dtype change; keep whatever you have)
    vox_np = vox.contiguous().cpu().numpy()          # [T, C, H, W]
    ts_us_np = (ts_s * 1e6).round().to(torch.long).cpu().numpy()  # int64

    # write arrays
    np.save(vox_path, vox_np)       # .npy
    np.save(ts_path, ts_us_np)      # .npy
    ...
    if write_meta_json:
        meta = {
            ...
            "shape": list(vox_np.shape),            # [T,C,H,W]
    ...
    print(f"💾 Saved cache:")
    print(f"   voxels -> {vox_path}  (shape={vox_np.shape}, dtype={vox_np.dtype})")
    print(f"   times  -> {ts_path}   (len={ts_us_np.shape[0]})")
    if write_meta_json:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # ======= GT-driven export (no LCM) =======
            # For each subsequence you loaded above:
            # ======= GT-driven consecutive intervals (no LCM) =======


            gt_times_all = gt_tracks[:, 1].astype(np.float64)
            gt_times = np.unique(gt_times_all)  # ascending, unique

            if gt_times.size < 2:
                print("Not enough GT timestamps to form intervals; skipping.")
            else:
                print(f"Processing {gt_times.size - 1} consecutive GT intervals")

                # where to save: per subsequence
                ...
                for i in range(len(gt_times) - 1):
                    t0 = float(gt_times[i])
                    t1 = float(gt_times[i + 1])

                    # half-open [t0, t1)
                    start_us = int(np.floor(t0 * 1e6))
                    end_us   = int(np.floor(t1 * 1e6))

                    # For STRICT non-overlap across files, keep margin 0 here.
                    # If you *do* want safety margin, you can expand start/end, but
                    # that will intentionally create overlaps across neighboring caches.

                    print(f"\n--- Interval {i+1}/{len(gt_times)-1}: "
                          f"[{t0:.6f}, {t1:.6f}) s "
                          f"-> [{start_us*1e-6:.6f}, {end_us*1e-6:.6f}) s")

                    combined_voxels, combined_timestamps = collect_window_half_open(
                        dataset=dataset,
                        start_us=start_us,
                        end_us=end_us,
                        debug=DEBUG_PRINTS,
                    ...
                    if combined_voxels is None:
                        print("  (no frames collected, skipping)")
                        continue

                    # print(f"  Saving {combined_voxels.shape[0]} frames to cache... {save_dir}")
                    
                    # print(f"name: {int(round(t0*1e6))}")
                    # save_cache_pt(
                    #     cache_dir=save_dir,
                    #     name=f"{int(round(t0*1e6))}",
                    #     start_us=start_us,
                    #     end_us=end_us,
                    #     margin_us=0,  # we used strict half-open; keep 0 for clarity in metadata
                    #     vox=combined_voxels,
                    #     ts_s=combined_timestamps,
                    # )
                    save_cache_npy(
                        cache_dir=save_dir,                 # same dir you used before
                        name=f"{int(round(t0*1e6))}",       # sequence/name tag for meta; keep as you like
                        start_us=start_us,
                        end_us=end_us,
                        margin_us=0,                        # you used strict half-open intervals
                        vox=combined_voxels,
                        ts_s=combined_timestamps,
                    )
