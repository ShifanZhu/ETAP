...
class TrackInterpolator:
    def __init__(self, track_data: np.ndarray, terminate_out_of_frame: bool = False, img_size: Optional[Tuple[int, int]] = None):
    ...
    def interpolate(self, track_idx: int, t_query: float) -> Optional[np.ndarray]:
    ...
    def interpolate_list(self, track_idx: int, t_query_list: np.ndarray) -> Optional[np.ndarray]:
        track_interpolator = self.track_interpolators[track_idx]
    ...
    def history(self, track_idx: int, t_query: float, dt_history: float) -> Optional[np.ndarray]:
        track_interpolator = self.track_interpolators[track_idx]
    ...
    def terminate_track(self, track_data: np.ndarray, img_size: Tuple[int, int]) -> np.ndarray:
        new_track_data = []
...
class TrackObserver:
    def __init__(self, t_init: Union[float, np.ndarray], u_centers_init: np.ndarray):
    ...
    def add_observation(self, t: float, u_centers: np.ndarray, mask: Optional[np.ndarray] = None) -> None:
    ...
    def get_interpolators(self) -> TrackInterpolator:
        return TrackInterpolator(self.track_data)

    def terminate_oob(self, img_size: Tuple[int, int], padding: float) -> None:
        """
        :param img_size: (H, W)
...
class TrackTriangulator:
    def __init__(
        self, track_data: np.ndarray, pose_interpolator: 'PoseInterpolator', 
        t_init: float, camera_matrix: np.ndarray, depths: Optional[np.ndarray] = None
    ):
    ...
    def get_corners(self, T_j_init: np.ndarray) -> np.ndarray:
        """
        Determine the 2D position of the features from the initial extraction step
        :param T_j_init: Transformation matrix from initial to current frame
        :return: 2D corner positions
        """
    ...
    def get_depths(self, T_j_init: np.ndarray) -> np.ndarray:
        """
        Determine the depths of the features from the initial extraction step
        :param T_j_init: Transformation matrix from initial to current frame
        :return: Depth values for each corner
        """
        corners_3D = (T_j_init @ self.corners_3D_homo.T).T
        corners_3D = corners_3D[:, :3]
        return corners_3D[:, 2]
...
class PoseInterpolator:
    def __init__(self, pose_data: np.ndarray, mode: str = "linear"):
        """
    ...
    def interpolate_colmap(self, t: float) -> np.ndarray:
        """
        Interpolate pose and convert to COLMAP format
        :param t: Query time at which to interpolate the pose
        :return: 7-element array with position and quaternion [tx, ty, tz, qx, qy, qz, qw]
        """
        if t < np.min(self.pose_data[:, 0]) or t > np.max(self.pose_data[:, 0]):
            print(
                f"Query time is {t}, but time range in pose data is [{np.min(self.pose_data[:, 0])}, {np.max(self.pose_data[:, 0])}]"
...
def retrieve_track_tuples(extra_dir: Path, track_name: str) -> List[Tuple[str, int]]:
    track_tuples = []
    for extra_seq_dir in tqdm(extra_dir.iterdir(), desc="Fetching track paths..."):
        ...
        track_path = extra_seq_dir / "tracks" / f"{track_name}.gt.txt"
        if not track_path.exists():
            continue

        # Store paths
        track_data = np.genfromtxt(str(track_path))
        n_tracks = len(np.unique(track_data[:, 0]))
        for track_idx in range(n_tracks):
            track_tuples.append((str(track_path), track_idx))

    return track_tuples


def compute_tracking_errors2(
    pred_track_data: np.ndarray,
    gt_track_data: np.ndarray,
    klt_track_data: np.ndarray,
    pose_interpolator: Optional['PoseInterpolator'] = None,
    camera_matrix: Optional[np.ndarray] = None,
    error_threshold: float = 5,
    n_tracks_dead: int = 0,
...
def read_txt_results(results_txt_path: Union[str, Path]) -> np.ndarray:
    """
    Parse an output txt file from E-KLT or Ours of data rows formatted [id, t, x, y]
    :param results_txt_path: Path to the results txt file
    :return: TrackInterpolator
    """
    return np.genfromtxt(str(results_txt_path))


def get_gt_corners(results_txt_path: Union[str, Path]) -> np.ndarray:
    """
    Get initial corners from EKLT results
    :param results_txt_path: Path to the results txt file
    :return: Array of initial corner positions
    """
    track_data = np.genfromtxt(str(results_txt_path))
