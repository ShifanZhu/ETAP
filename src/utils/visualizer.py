...
def draw_line(rgb, coord_y, coord_x, color, linewidth):
    draw = ImageDraw.Draw(rgb, 'RGBA')  # Use RGBA mode for better antialiasing
    draw.line(
        (coord_y[0], coord_y[1], coord_x[0], coord_x[1]),
        fill=color,
        width=linewidth,
        joint="round"  # Add round joints to reduce artifacts at line intersections
    )
    return rgb
...
class Visualizer:
    def __init__(
        ...
        self.fps = fps
        self.consistent_colors = consistent_colors
        self.color = None
    ...
    def visualize(
        ...
        if mask_segm_tracks:
            ...
            visibility = visibility[:, :, segm_mask != 0] if visibility is not None else None
            segm_mask = None
    ...
    def draw_tracks_on_video(
        ...
        valid_mask = ~torch.all(torch.isnan(tracks)[0], dim=-1).cpu().numpy()
        ...
        if self.mode == "optical_flow":
            ...
            if self.mode == "rainbow":
                if self.color is None or not self.consistent_colors:
                    y_min, y_max = (
                    ...
                    if self.color is None:
                        self.color = vector_colors
                else:
                    vector_colors = self.color
            else:
    ...
    def _draw_pred_tracks(
        ...
        T, N, _ = tracks.shape
        # Convert to PIL Image once at the start
        rgb = Image.fromarray(np.uint8(rgb))
        
        # Create a single composite image for all tracks
        for s in range(T - 1):
            vector_color = vector_colors[s]
            time_alpha = (s / T) ** 2 if self.tracks_leave_trace > 0 else 1.0
            ...
            for i in range(N):
                ...
                if coord_y[0] != 0 and coord_y[1] != 0 and coord_x[0] != 0 and coord_x[1] != 0:
                    ...
                    color = (*vector_color[i].astype(int),)  # Convert to tuple
                    rgb = draw_line(
                        rgb,
                        coord_y,
                        coord_x,
