...
# Modified by F. Hamann (TU Berlin)
# Copyright (c) 2025 F. Hamann (TU Berlin)
import torch
...
from .core.cotracker.blocks import (
    Mlp,
    BasicEncoder,
    AttnBlock,
    CorrBlock,
...
class Etap(nn.Module):
    def __init__(
        ...
        cache_features=False,  # TODO: remove cache_features
        num_in_channels=3,
        **kwargs,
    ):
        super(Etap, self).__init__()
        self.window_len = window_len
        self.stride = stride  # factor between input and feature map resolution
        self.hidden_dim = 256
        self.latent_dim = 128
        self.add_space_attn = add_space_attn

        self.fnet = BasicEncoder(output_dim=self.latent_dim,
                                 input_dim=num_in_channels)
    ...
    def forward_window(
        ...
        features_at_coords = corr_block.sample_fmap(coords) if self.cache_features else None
        vis_pred = self.vis_predictor(track_feat).reshape(B, S, N)
        return coord_preds, vis_pred, features_at_coords, track_feat
    ...
    def forward(self, video, queries, iters=4, is_train=False, is_online=False, gt_flow=None):
        """Predict tracks
        ...
        assert not is_online or not self.cache_features, "Cache features not supported in online mode."
        ...
        features_at_coords = torch.zeros((B, T, N, 4 * self.latent_dim), device=device) if self.cache_features else None
        all_track_feat = torch.zeros((B, T, N, self.latent_dim), device=device) if self.cache_features else None
        ...
        for ind in indices:
            ...
            coords, vis, features, converged_track_feat = self.forward_window(
            ...
            if self.cache_features:
                features_at_coords[:, ind : ind + S] = features[:, :S_trimmed]
                all_track_feat[:, ind : ind + S] = converged_track_feat[:, :S_trimmed]
            if is_train:
        ...
        result = {
            "coords_predicted": coords_predicted,
            "vis_predicted": vis_predicted,
            "train_data": train_data,
        }

        if self.cache_features:
            result["features_at_coords"] = features_at_coords
